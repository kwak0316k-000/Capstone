...
| 양쪽 귀의 무선 이어버드(에어팟 스타일) + 스마트폰 | 배달 리뷰를 집요하게 분석하는 습관, 영상 편집/미디어 감각 (초안의 목에 건 유선 이어폰이 어색해 무선으로 교체) |
...
| 요소 | 반영한 실제 데이터 |
|---|---|
| 크림색 니트 카디건 + 원피스, 오른쪽 어깨에만 멘 캔버스 토트백 | 소품샵 구경, 빽빽한 일정 없이 발길 닿는 대로 움직이는 성향 (가방을 한쪽 어깨에만 배치해 실제 착용 각도를 정확히 반영) |
...
4. **4차 시도**: "2등신, 큰 머리+작은 몸, 사실적 인체 비율 아님"이라는 스타일 지시를 방향 수정 지시와 함께 넣어서야 원하는 결과(스타일 O, 방향 O)를 얻었다.
5. **5차 시도(비대칭 디테일 오류 발견)**: 4차 결과물을 자세히 보니, 오른쪽·왼쪽 옆모습이 진짜 다른 각도가 아니라 "거울상"으로 처리되어 있었다. 가르마처럼 좌우가 원래 다른 머리 스타일이라면, 실제 오른쪽 옆모습과 왼쪽 옆모습은 이마 노출 정도가 서로 달라야 하는데, 단순 거울상 지시는 이걸 그대로 뒤집어버려 "같은 쪽 얼굴을 좌우만 반전한 가짜 옆모습"을 만들고 있었다. 최수민의 에코백(원래 오른쪽 어깨에만 있어야 함)도 양쪽 옆모습에 전부 나타나는 같은 문제를 보였다.
6. **6차 시도**: "거울상" 지시를 완전히 빼고, 오른쪽·왼쪽 옆모습을 각각 독립적으로 서술했다 — 예: "오른쪽 옆모습은 앞머리가 눈썹을 거의 덮을 정도, 왼쪽 옆모습은 눈썹이 뚜렷하게 보임"처럼 비대칭 자체를 구체적으로 지정. 가방도 "오른쪽 옆모습에서만 뚜렷이 보이고, 왼쪽 옆모습에서는 몸에 가려 안 보이거나 끈만 살짝"이라고 명시했다. 김재성의 목에 건 유선 이어폰도 이 과정에서 어색하다는 피드백을 받아 무선 이어버드(에어팟 스타일)로 교체했다.
7. **7차 시도(뒷모습 귀 오류 발견)**: 6차 결과물의 김재성 뒷모습을 자세히 보니, 정면에서 보이던 "에어팟 낀 귀"가 뒤통수에 그대로 옮겨붙어 있었다 — 앞모습의 귀 이미지를 복사해서 뒤에 붙인 것처럼 보이는 기괴한 결과였다. 처음엔 "귀를 아예 안 보이게 하라"고 단순화했지만, 김재성은 머리가 짧은 남성 캐릭터라 실제로는 뒤에서도 귀 자체는 보이는 게 맞는 구조였다 — 오히려 김소희·최수민(머리가 긴 캐릭터)은 같은 지시 없이도 뒤통수 쪽 귀 모양을 알아서 정확히 그려냈다.
8. **8차 시도(최종)**: "귀"와 "에어팟"을 분리해서 지시했다 — 귀 자체는 짧은 머리 구조상 뒤에서도 보이는 게 맞으므로 뒤통수 쪽 정확한 귀 모양으로 그리게 하고, 에어팟만 "원래 앞을 향해 귓속에 꽂히는 구조라 뒤에서는 귀에 가려 안 보인다"고 명시해 제외시켰다. 이렇게 하니 귀는 자연스럽게 보이면서 에어팟만 사라진, 해부학적으로 맞는 뒷모습이 나왔다.

**배운 점 (갱신)**: "안 보이게 하라"는 뭉뚱그린 지시보다 "왜 안 보여야 하는지(에어팟은 앞을 향한 구조라서)"와 "그 캐릭터의 실제 구조상 무엇은 보이는 게 맞는지(짧은 머리라 귀 자체는 보임)"를 함께 설명하는 게 훨씬 정확한 결과를 만들었다. 같은 지시라도 캐릭터마다 머리 길이 같은 구조적 차이 때문에 결과가 다르게 나올 수 있다는 것도 확인했다 — 세 캐릭터에 동일한 프롬프트 틀을 썼다고 항상 같은 문제가 생기는 건 아니었다.
