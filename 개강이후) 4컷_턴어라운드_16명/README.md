...
> 정면/오른쪽/왼쪽/뒷모습 4컷 턴어라운드 캐릭터를 생성한 과정과 결과를 정리했다.
...
그래서 방식을 바꿨다 — **Claude(나)가 각 페르소나의 CSV를 직접 읽고, 그 사람만의 완성된 프롬프트를
...
| 뒷모습에 정면의 귀(에어팟 낀 채) 모양이 그대로 복붙됨 | "귀는 짧은 머리 구조상 보이는 게 맞지만, 에어팟은 뒤에서 안 보인다"로 **귀와 액세서리를 분리해서 지시** |
