...
## 1. 캐릭터 디자인 — 김소희 (4방향 턴어라운드, v2)

`assets/kim_sohui_turnaround.svg` — 정면/오른쪽/왼쪽/뒷모습. 눈·눈썹·코·입 등 이목구비와 손, 후드 디테일, 음영(2단 톤)을 추가한 버전입니다.

**디자인 근거** (실제 페르소나 필드 기반):
| 요소 | 반영한 실제 데이터 |
|---|---|
...
| 절도 있는 실루엣, 다부진 체형 | `professional_persona`: 육군 부사관, 평정심·책임감 있는 성격 |

> **여전히 남아있는 한계**: 이 환경(Claude 코드 샌드박스)은 래스터 이미지 생성 API가 없어 SVG 벡터로 제작했습니다. v2는 이목구비·음영·손을 추가해 완성도를 높였지만, 여전히 "플랫 일러스트" 톤이며 사진처럼 사실적인 스타일은 아닙니다. 더 높은 완성도(채색 디테일, 자연스러운 명암, 실사에 가까운 느낌)가 필요하면 아래 프롬프트를 Midjourney/DALL-E/Stable Diffusion 등에 입력해 재현하세요.

**재현용 이미지 생성 프롬프트**:
```
Character turnaround sheet, front/right/left/back view, Korean woman in her early 30s,
athletic build, dark brown hair in a neat ponytail, soft round face with warm brown eyes,
wearing an oversized sage green hoodie with drawstrings and black leggings, holding a
small monstera plant in a terracotta pot with both hands, soft flat illustration style
with subtle two-tone shading, clean line art, neutral white background, consistent
character design across all four views, semi-realistic proportions
```
